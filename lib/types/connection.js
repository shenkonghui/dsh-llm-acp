...
export class AcpConnection {
    ...
    ensureAuthenticated() {
        ...
        this.authRound = (async () => {
            ...
            const settled = await Promise.race([
                ...
                sleep(this.spec.authTimeoutMs).then(() => ({ done: false, error: undefined })),
            ]);
            if (settled.done) {
            ...
            this.spec.onWarn?.(`llm-acp: interactive authentication for "${this.spec.command}" is still pending after ${this.spec.authTimeoutMs}ms`
    ...
    async withAuthRetry(label, call) {
        try {
            ...
            if (this.authRound !== undefined || this.authMethods === undefined || this.authMethods.length === 0)
                throw error;
    ...
    enqueueUpdate(params) {
        ...
        const preview = isChunk
            ...
            : '';
        this.traceEvent('recv', 'session/update', `${update.sessionUpdate} sessionId=${params.sessionId}${preview}`, isChunk ? `update:${update.sessionUpdate}:${params.sessionId}` : undefined);
