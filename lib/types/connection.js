...
export class AcpConnection {
    ...
    async authenticateIfNeeded(initResult) {
        ...
        const settled = await Promise.race([
            ...
            sleep(KEYLESS_AUTH_TIMEOUT_MS).then(() => ({ done: false, error: undefined })),
        ]);
        if (settled.done) {
        ...
        this.spec.onWarn?.(`llm-acp: interactive authentication for "${this.spec.command}" is still pending after ${KEYLESS_AUTH_TIMEOUT_MS}ms`
