...
async function loadProviderModels(api, settingsNs, providerRoute) {
    try {
        ...
        if (!response.result.ok)
            return [];
        return response.result.value.models.map((m) => ({ id: m.id, name: m.name ?? m.id }));
    }
    catch {
        return [];
    }
