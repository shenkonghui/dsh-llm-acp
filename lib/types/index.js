...
import { AcpConnection, DEFAULT_AUTH_TIMEOUT_MS, DEFAULT_DISPOSE_EOF_GRACE_MS, DEFAULT_DISPOSE_GRACE_MS, DEFAULT_INIT_TIMEOUT_MS, DEFAULT_INTERACTIVE_AUTH_TIMEOUT_MS, DEFAULT_SESSION_TIMEOUT_MS, } from "./connection.js";
import registryData from './registry.json' with { type: 'json' };
export { AcpAdapter } from "./adapter.js";
export { AcpConnection, DEFAULT_AUTH_TIMEOUT_MS, DEFAULT_DISPOSE_EOF_GRACE_MS, DEFAULT_DISPOSE_GRACE_MS, DEFAULT_INIT_TIMEOUT_MS, DEFAULT_INTERACTIVE_AUTH_TIMEOUT_MS, DEFAULT_SESSION_TIMEOUT_MS, } from "./connection.js";
 ...
 */
function acpToolCallRecorder(session, onWarn) {
...
export function apply(ctx, config) {
    ...
    function createServer(serverId, server) {
        ...
        const adapter = new AcpAdapter({
            ...
            includeHarnessPrompt: () => currentSettings().includeHarnessPrompt ?? resolved.includeHarnessPrompt ?? false,
            includeRuntimeContext: () => currentSettings().includeRuntimeContext ?? resolved.includeRuntimeContext ?? false,
            ...
            resolveSessionMode: () => {
                ...
                if (modeMap === undefined || Object.keys(modeMap).length === 0)
                    return undefined;
                const agent = ctx.get('agents')?.currentInitiator();
                const session = agent?.session;
                if (session === undefined)
            ...
            permissionRequester: () => {
                ...
                if (approval === undefined && sandboxPolicy === undefined && permissionPresets === undefined)
                    return undefined;
                ...
                return async ({ title, signal, optionLabels }) => {
                    ...
                    if (permissionPresets !== undefined) {
                        ...
                        if (permissionPresets.names.includes(current))
                            preset = permissionPresets.resolve(current);
                    }
