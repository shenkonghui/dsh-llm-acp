		...
		* which would hide unselected models from the multi-select editor. */
		async function loadProviderModels(api, settingsNs, providerRoute) {
			try {
				...
				return response.result.value.models.map((m) => ({
					id: m.id,
					name: m.name ?? m.id
				}));
			} catch {
