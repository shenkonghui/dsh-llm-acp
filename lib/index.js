...
var AcpConnection = class {
	...
	ensureAuthenticated() {
		...
		this.authRound = (async () => {
			...
			if (settled.done) {
				...
				} else this.traceEvent("recv", "authenticate", `methodId=${method.id} ok`, void 0, { methodId: method.id });
				return;
			}
	...
	*/
	...
	enqueueUpdate(params) {
		...
		if (entry === void 0) {
			this.traceEvent("recv", "session/update-dropped", `${update.sessionUpdate} sessionId=${params.sessionId}`, `drop:${update.sessionUpdate}:${params.sessionId}`, params);
