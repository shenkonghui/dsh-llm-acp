...
function isAuthRequiredError(error) {
	...
	return Reflect.get(error, "code") === -32e3 || /^authentication required/i.test(error.message);
}
	...
	enqueueUpdate(params) {
		...
		if (phase === "idle") {
			if (entry.idleTimer === void 0) entry.idleTimer = setTimeout(() => {
				...
				this.conn.cancel({ sessionId: params.sessionId }).catch(() => {});
				this.spec.onWedged?.(`prompt went idle without answering (session ${params.sessionId})`);
				entry.queue.push({
					kind: "error",
					error: /* @__PURE__ */ new Error(`llm-acp: agent went idle without answering session/prompt for session ${params.sessionId} — the server dropped the turn`)
				});
				this.signal(entry);
