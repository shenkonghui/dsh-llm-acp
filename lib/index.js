	...
	*/
	...
	async resolveModel(provider, model) {
		...
		return {
			provider,
			id: model,
			name: this.models.find((m) => m.id === model)?.name ?? model
		};
	}
...
function pickAuthMethod(methods) {
	return methods.find((m) => /api[-_]?key|token|credential/i.test(`${m.id} ${m.name}`)) ?? methods[0];
}
...
function describePermissionToolCall(toolCall, options) {
	...
	if (options !== void 0 && options.length > 0) {
		const labels = options.map((o) => o.name).filter((n) => typeof n === "string" && n.length > 0);
...
var AcpConnection = class {
	...
	* so per-token stream chunks do not flood the small buffer. */
	traceEvent(dir, method, summary, collapseKey) {
	...
	*/
	ensureAuthenticated() {
		if (this.authRound !== void 0) return this.authRound;
		const method = this.authMethods?.[0];
		if (method === void 0) return Promise.resolve();
		...
		this.authRound = (async () => {
			...
			const settled = await Promise.race([attempt.then(() => ({
				...
				error
			})), sleep(this.spec.authTimeoutMs).then(() => ({
				done: false,
				error: void 0
			}))]);
			if (settled.done) {
				...
				} else this.traceEvent("recv", "authenticate", `methodId=${method.id} ok`);
				return;
			}
			const url = this.pendingAuthUrl;
			this.spec.onWarn?.(`llm-acp: interactive authentication for "${this.spec.command}" is still pending after ${this.spec.authTimeoutMs}ms` + (url !== void 0 ? ` — complete the login in a browser: ${url}` : ""));
		})();
	...
	*/
	async withAuthRetry(label, call) {
		try {
			...
			if (this.authRound !== void 0 || this.authMethods === void 0 || this.authMethods.length === 0) throw error;
	...
	enqueueUpdate(params) {
		...
		if (entry === void 0) {
			this.traceEvent("recv", "session/update-dropped", `${update.sessionUpdate} sessionId=${params.sessionId}`, `drop:${update.sessionUpdate}:${params.sessionId}`);
		...
		this.traceEvent("recv", "session/update", `${update.sessionUpdate} sessionId=${params.sessionId}${preview}`, isChunk ? `update:${update.sessionUpdate}:${params.sessionId}` : void 0);
	...
	* `options` array of leaf values, so flatten one level before collecting. */
	extractModels(options) {
		...
		const models = [];
		for (const opt of selectOptions) if ("value" in opt && typeof opt.value === "string" && typeof opt.name === "string") models.push({
			id: opt.value,
			name: opt.name
		});
		else if ("group" in opt && Array.isArray(opt.options)) {
			for (const leaf of opt.options) if ("value" in leaf && typeof leaf.value === "string" && typeof leaf.name === "string") models.push({
				id: leaf.value,
				name: leaf.name
			});
		}
...
function apply(ctx, config) {
			...
			permissionRequester: () => {
				...
				if (permissionPresets !== void 0) {
					...
					if (permissionPresets.names.includes(current)) {
						...
						if (preset.sandbox === "danger-full-access" && preset.approval === "never") return async () => "allow";
					}
				}
	...
	ctx.effect(() => ctx.llm.registerModelDiscovery(NS, async (request, _signal) => {
		...
		if (provider.startsWith(AUTH_PREFIX)) {
			const serverId = provider.slice(9);
			const url = active.get(serverId)?.connection.getPendingAuthUrl();
...
//#endregion
export { AcpAdapter, AcpConnection, Config, DEFAULT_AUTH_TIMEOUT_MS, DEFAULT_DISPOSE_EOF_GRACE_MS, DEFAULT_DISPOSE_GRACE_MS, DEFAULT_INIT_TIMEOUT_MS, DEFAULT_SESSION_TIMEOUT_MS, registry_default as acpRegistry, apply, inject, name };
