...
import { LlmAdapter, LlmError, ToolCallId, createToolResultMessage } from "@deepseek-ai/dsh-llm";
	...
	*/
	async *stream(options) {
		...
		if (existing === void 0 && dshSessionId !== void 0) {
			...
			if (stored !== void 0 && await this.config.connection.loadSession(stored.acpSessionId)) {
...
var AcpConnection = class {
	...
	*/
	...
	enqueueUpdate(params) {
		...
		} else if (update.sessionUpdate === "plan") {} else if (update.sessionUpdate === "user_message_chunk") {}
		this.trackAgentPhase(params.sessionId, update);
		this.signal(entry);
	}
	/**
