...
var AcpAdapter = class extends LlmAdapter {
	...
	constructor(config) {
		...
		this.models = allow !== void 0 && allow.length > 0 && !allow.includes(config.defaultModel.id) ? [] : fallback;
	...
	async discoverModels() {
		try {
			...
			if (discovered !== void 0 && discovered.length > 0) {
				...
				this.models = allow !== void 0 && allow.length > 0 ? all.filter((m) => allow.includes(m.id)) : all;
			}
		} catch {}
	}
...
*/
const KEYLESS_AUTH_TIMEOUT_MS = 15e3;
...
var AcpConnection = class {
	...
	*/
	async authenticateIfNeeded(initResult) {
		...
		const settled = await Promise.race([attempt.then(() => ({
			...
			error
		})), sleep(KEYLESS_AUTH_TIMEOUT_MS).then(() => ({
			done: false,
			error: void 0
		}))]);
		if (settled.done) {
		...
		this.spec.onWarn?.(`llm-acp: interactive authentication for "${this.spec.command}" is still pending after ${KEYLESS_AUTH_TIMEOUT_MS}ms` + (url !== void 0 ? ` — complete the login in a browser: ${url}` : ""));
	}
	...
	async requestPermission(params) {
		...
		let decision;
		try {
			const title = typeof params.toolCall.title === "string" && params.toolCall.title.length > 0 ? params.toolCall.title : "ACP operation";
...
function apply(ctx, config) {
	...
	ctx.effect(() => ctx.llm.registerModelDiscovery(NS, async (request, _signal) => {
		...
		if (provider.startsWith(INFO_PREFIX)) {
			...
			try {
				await Promise.race([server.connection.ready, new Promise((resolve) => setTimeout(() => resolve(void 0), 1e4))]);
			} catch {
				return [];
			}
